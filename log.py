...
class LogFormatter(logging.Formatter):
    def __init__(self, local_rank, color=True) -> None:
        ...
        if local_rank == -1:
            ...
            self._fmt = "%(color)s[%(levelname)1.1s Local{} %(asctime)s %(module)s:%(lineno)d]%(end_color)s %(message)s".format(local_rank)
        colors = {
    ...
    def get_file_formatter(self):
        if self.local_rank == -1:
            ...
            format_str = """[%(levelname)1.1s Local{} %(asctime)s %(module)s:%(lineno)d]%(message)s""".format(self.local_rank)
...
def test():
    logger = create_logger("what.log", local_rank=1)
    logger.debug("debug")
    logger.info("info")
